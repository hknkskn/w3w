...
export function TrainingCenter() {
    ...
    const handleUpgrade = async (e: React.MouseEvent, id: number) => {
        ...
        const confirmed = await idsConfirm(`Initiate Q${facility.quality + 1} structural upgrade for ${facility.upgradeCostSupra} SUPRA?`, "Infrastructure Upgrade");
                ...
                <div className="divide-y divide-slate-700/30 min-h-[100px] flex flex-col items-stretch">
                    {facilities.length === 0 ? (
                        <div className="py-20 text-center">
                            <div className="text-slate-500 font-bold uppercase tracking-[0.2em] mb-2 scale-150 opacity-20">No Infrastructure</div>
                            <p className="text-slate-600 text-[10px] font-medium uppercase tracking-widest">Building registry returned empty vector.</p>
                        </div>
                    ) : (
                        facilities.map((f) => {
                                        ...
                                        <div className="relative">
                                            <div className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${isSelected
                                                ...
                                                <div className="absolute inset-0 bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center overflow-hidden">
                                                    <img
                                                        src={quality >= 5 ? "/icons/trainingbuilds/q5.webp" : `/icons/trainingbuilds/q${quality}.webp`}
                                            ...
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                {!f.isMaxLevel && (
                                                    ...
                                                    >
                                                        ...
                                                        <span>UPGRADE ({user?.isAdmin ? 'FREE' : `${f.upgradeCostSupra?.toLocaleString()} S`})</span>
                                                    </button>
                                                )}
                                            </div>
